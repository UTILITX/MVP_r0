...
export default function MapWithDrawing({
  ...
  const toggleLayerVisibility = useCallback((layerType: keyof typeof layerVisibility) => {
    const map = mapInstanceRef.current
    if (!map) return

    setLayerVisibility((prev) => {
      const newVisibility = !prev[layerType]

      // Get the appropriate layer ref
      let layerRef: L.LayerGroup | null = null
      switch (layerType) {
        case "workArea":
          layerRef = workAreaLayerRef.current
          break
        case "record":
          layerRef = recordLayerRef.current
          break
        case "existingRecords":
          layerRef = recordLayerRef.current
          break
        case "georef":
          layerRef = georefLayerRef.current
          break
        case "bubbles":
          layerRef = bubblesLayerRef.current
          break
        case "shapes":
          layerRef = shapesLayerRef.current
          break
      }
      ...
      return { ...prev, [layerType]: newVisibility }
    })
  }, [])
    ...
    <div className="flex h-full w-full">
      ...
      <div className="w-1/3 bg-white border-r border-gray-200 overflow-y-auto">
        <div className="p-4 space-y-4">
          ...
          <div className="bg-gradient-to-br from-slate-50 to-gray-50 p-4 rounded-lg border border-gray-200">
            <div className="font-semibold text-sm mb-3 text-gray-800">Layer Visibility</div>
            <div className="space-y-2">
              <label className="flex items-center justify-between p-2 rounded hover:bg-white transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
                  <span className="text-lg">🟢</span>
                  <span className="text-sm text-gray-700">Work Areas</span>
                ...
                <input
                  type="checkbox"
                  checked={layerVisibility.workArea}
                  onChange={() => toggleLayerVisibility("workArea")}
              ...
              <label className="flex items-center justify-between p-2 rounded hover:bg-white transition-colors cursor-pointer">
                ...
                <input
                  type="checkbox"
                  checked={layerVisibility.record}
                  onChange={() => toggleLayerVisibility("record")}
              ...
              <label className="flex items-center justify-between p-2 rounded hover:bg-white transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
                  <span className="text-lg">📍</span>
                  <span className="text-sm text-gray-700">Existing Records</span>
                  <span className="text-xs text-gray-500">({recordLayerRef.current?.getLayers().length || 0})</span>
                </div>
                <input
                  type="checkbox"
                  checked={layerVisibility.existingRecords}
                  onChange={() => toggleLayerVisibility("existingRecords")}
              ...
              <label className="flex items-center justify-between p-2 rounded hover:bg-white transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
                  <span className="text-lg">🎯</span>
                  <span className="text-sm text-gray-700">Georef Points</span>
                  <span className="text-xs text-gray-500">({georefLayerRef.current?.getLayers().length || 0})</span>
                </div>
                <input
                  type="checkbox"
                  checked={layerVisibility.georef}
                  onChange={() => toggleLayerVisibility("georef")}
              ...
              <label className="flex items-center justify-between p-2 rounded hover:bg-white transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
                  <span className="text-lg">⭕</span>
                  <span className="text-sm text-gray-700">Record Markers</span>
                  <span className="text-xs text-gray-500">({bubblesLayerRef.current?.getLayers().length || 0})</span>
                </div>
                <input
                  type="checkbox"
                  checked={layerVisibility.bubbles}
                  onChange={() => toggleLayerVisibility("bubbles")}
                  className="w-4 h-4 text-blue-500 rounded focus:ring-blue-500"
                />
              </label>
              ...
              <label className="flex items-center justify-between p-2 rounded hover:bg-white transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
                  <span className="text-lg">🔶</span>
                  <span className="text-sm text-gray-700">Record Boundaries</span>
                  <span className="text-xs text-gray-500">({shapesLayerRef.current?.getLayers().length || 0})</span>
                </div>
                <input
                  type="checkbox"
